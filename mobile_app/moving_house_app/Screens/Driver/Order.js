...
function Order({ navigation }) {
                        ...
                        await axios.get(`${api_url}/v1/order/viewAllOrder`).then((data) => {
                            ...
                            dataOrder &&
                                dataOrder.forEach((item, index) => {
                                    if (item.driver_name.includes(fullname_driver) && status_driver === "Sẵn sàng" && item.status === "Đã hoàn thành") {
    ...
    const navigation_to_detailOrder = (order_detail_id, driver_name, order_id, fullname_driver, quantity_driver, customer_id, from_location, to_location, date_start, time_start) => {
        navigation.navigate('OrderDetail', { data_order: order_detail_id, driver_name: driver_name, order_id: order_id, fullname_driver: fullname_driver, quantity_driver: quantity_driver, customer_id, from_location, to_location, date_start, time_start })
    }
