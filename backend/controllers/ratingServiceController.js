...
const ratingServiceController = {
    ...
    //Tìm kiếm đánh giá dịch vụ
    findRatingService: async (req, res) => {
        try {
            const { customerName, orderId } = req.body;

            //Gọi dữ liệu đánh giá dịch vụ
            const call_api_rating_service = await RatingService.find();
            const data_rating_service = call_api_rating_service;

            const data_all = data_rating_service.map((item, index) => {
                return {
                    customer_name: item.customer_name,
...
module.exports = ratingServiceController;